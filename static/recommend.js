$(function() {
    // Enter button is disbaled unless we type anything in the search box
    ...
    $('.movie-button').on('click',function(){
      var my_api_key = '2a38eb1441f62cd41450c2c555579f99';  //API key generated from tmdb
      var title = $('.movie').val();
      if (title=="") {
  ...
  function recommendcard(e){
    var my_api_key = '2a38eb1441f62cd41450c2c555579f99';
    var title = e.getAttribute('title'); 
    load_details(my_api_key,title);
  }
  
  // Getting the basic details of movie from API
  ...
  // Passing the movie name to get similar movies suggestion from python flask
  ...
  // Getting all the details of movie via movie id
  ...
  function get_individual_cast(movie_cast,my_api_key) {
    ...
    function get_trailer(movie_id, my_api_key){
      $.ajax({
        ...
        async: false,
        success: function (movie_trailer){
         embed.push(movie_trailer.trailer);
        }
      })
    }
  ...
  // getting the posters for all the recommended movies
